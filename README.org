...
- Supports GKI-compatible devices (kernel versions 5.10 to 6.18) and can use
  fork-specific LKMs from KernelSU, KernelSU-Next, and backslashxx.
- ReSukiSU manager/ksud support is available, but ReSukiSU currently does not
  publish standalone LKM assets. Select a `kernelsu.ko` built for the target
  kernel when using it.
- YAAP and other non-GKI kernels cannot use a generic GKI `.ko` safely. An LKM
  must be built against the exact kernel configuration and exported symbols;
  this application cannot make an incompatible module compatible.
...
- *Fork-aware modules:* Select KernelSU, KernelSU-Next, ReSukiSU, or backslashxx and choose the matching KMI.
...
- [[https://github.com/ReSukiSU/ReSukiSU][ReSukiSU]] and [[https://github.com/backslashxx/KernelSU][backslashxx/KernelSU]] for their fork-specific userspace and LKM releases.
